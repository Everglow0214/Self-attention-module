...
class Res_SE_BasicBlock(nn.Module):
	expansion = 1
	...
	def forward(self, x):
		...
		out = out.view(out.size(0), out.size(1)) # NxC
		out = self.fc1(out)
		out = self.relu(out)
		out = self.fc2(out)
...
class Res_SE_Bottleneck(nn.Module):
	expansion = 4
	...
	def __init__(self, in_channels, out_channels, stride=1, downsample=None, reduction=16):
		...
		self.average_pool = nn.AdaptiveAvgPool2d(1)
		# 2 fully connected (FC) layers
		self.fc1 = nn.Linear(out_channels*4, out_channels*4//reduction, bias=False)
		self.fc2 = nn.Linear(out_channels*4//reduction, out_channels*4, bias=False)
		self.sigmoid = nn.Sigmoid()
		#### squeeze-excitation part ####
...
def resnet34_se(pretrained=False, **kwargs):
	model = ResNet_SE(Res_SE_BasicBlock, [3,4,6,3], **kwargs)
	flops, params = profile(model, input_size=(1, 3, 32,32),
		custom_ops={ResNet_SE: model})
	print ("flops & params:", flops, params)
	return model

def resnet50_se(pretrained=False, **kwargs):
	model = ResNet_SE(Res_SE_Bottleneck, [3,4,6,3], **kwargs)
	flops, params = profile(model, input_size=(1, 3, 32,32),
		custom_ops={ResNet_SE: model})
	print ("flops & params:", flops, params)
	return model